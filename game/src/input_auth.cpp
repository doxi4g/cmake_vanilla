...
void CInputAuth::Login(LPDESC d, const char * c_pData)
{
	...
	//pepper
	if(strcmp("peppertest12345", s_pepper) != 0)
	{
		std::string newpass = std::string(szPasswd);
		newpass.append(s_pepper);
		// CHANNEL_SERVICE_LOGIN
		DBManager::instance().ReturnQuery(QID_AUTH_LOGIN, dwKey, p, 
				"SELECT %s('%s'),password,securitycode,social_id,id,status,availDt - NOW() > 0,"
				...
				" FROM account WHERE login='%s'", mysql_pass_func.c_str(),
				newpass.c_str(), szLogin);
	} else {
		// CHANNEL_SERVICE_LOGIN
		DBManager::instance().ReturnQuery(QID_AUTH_LOGIN, dwKey, p, 
				"SELECT %s('%s'),password,securitycode,social_id,id,status,availDt - NOW() > 0,"
