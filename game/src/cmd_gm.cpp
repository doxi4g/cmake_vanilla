...
ACMD(do_promote)
{
	...
	char a1[256];
	char a2[256];
	two_arguments(argument, a1, sizeof(a1), a2, sizeof(a2));
	if(!*a1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "promotion error: Expected player name!");
		return;
	}
	LPCHARACTER target = CHARACTER_MANAGER::instance().FindPC(a1);
	if(!target)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "promotion error: Character not found!");
		return;
	}
	if(!*a2)
	{
		//force elevation of no arg for promtion is given
		target->SetElevation(true);
		ch->ChatPacket(CHAT_TYPE_INFO, "promotion success (elevated)!");
		target->ChatPacket(CHAT_TYPE_INFO, "you've been promoted (elevated)!");
		return;
	}
	if(strcmp(a2, "GM_PLAYER"))
	{
		target->SetGMLevelForce(GM_PLAYER);
	} else if(strcmp(a2, "GM_LOW_WIZARD"))
	{
		target->SetGMLevelForce(GM_LOW_WIZARD);
	} else if(strcmp(a2, "GM_HIGH_WIZARD"))
	{
		target->SetGMLevelForce(GM_HIGH_WIZARD);
	} else if(strcmp(a2, "GM_GOD"))
	{
		target->SetGMLevelForce(GM_GOD);
	} else if(strcmp(a2, "GM_IMPLEMENTOR"))
	{
		target->SetGMLevelForce(GM_IMPLEMENTOR);
	} else
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "promotion error: no valid promotion value!");
		return;
	}
	ch->ChatPacket(CHAT_TYPE_INFO, "promotion success!");
	target->ChatPacket(CHAT_TYPE_INFO, "you've been promoted!");
}
