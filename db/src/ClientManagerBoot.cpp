...
//			isNameFile = false;
		...
		std::unique_ptr<SQLMsg> sqlMsg(CDBManager::instance().DirectQuery(query.str().c_str(), 0)); 
		if (sqlMsg.get() == nullptr) { 
			sys_err("libdb::InitializeMobTable", __LINE__, "Could not load mob_proto. Query failed!"); 
			return false; 
		} 
		std::unique_ptr<MYSQL_RES> pRes(sqlMsg->Get()->pSQLResult); 
...
void CClientManager::CheckItemTable()
{
	...
	std::unique_ptr<MYSQL_RES> pRes(sqlMsg->vec_pkResult[0]->pSQLResult); 

	if (pRes.get() == nullptr) { 
		//vnum_range isn't there!
		everything_ok = false;
	} else if(pRes.get()) {
		if (mysql_num_rows(pRes.get()) != 0)
		{
			everything_ok = true;
		}
	} else {
	...
	if(everything_ok==false)
	{
		...
		sqlMsg.reset(CDBManager::instance().DirectQuery(query2.str().c_str(), 0)); 
		if (sqlMsg.get() == nullptr) { 
